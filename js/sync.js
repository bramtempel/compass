...
export async function loadCached() {
  const [index, vecBuf, suggestions, library, images] = await Promise.all([
    idbGet('index'), idbGet('vectors_buf'), idbGet('suggestions'), idbGet('library'), idbGet('images')]);
  if (!index || !vecBuf) return null;
  return { index, vectors: decodeF16(vecBuf), suggestions, library, images };
}
...
export async function syncFromDrive(token, onProgress = () => {}) {
  ...
  let suggestions = null, library = null, images = null;
  ...
  if (imgId)  { onProgress('images', 90);      try { images = await (await driveDownload(imgId, token)).json(); } catch {} }
